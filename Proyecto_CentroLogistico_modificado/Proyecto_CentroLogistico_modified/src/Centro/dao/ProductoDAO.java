...
public class ProductoDAO {
    ...
    /**
     * Búsqueda general dentro de un tipo de producto. Filtra por coincidencia
     * en cualquier campo relevante: nombre, código, INVIMA, lote, ubicación o
     * presentación. Equivale a {@link #buscarConLotesPorCampo(String, int, String)
     * buscarConLotesPorCampo(termino, idTipo, "TODOS")}.
     */
    public List<Object[]> buscarConLotes(String termino, int idTipo) {
        return buscarConLotesPorCampo(termino, idTipo, "TODOS");
    }

    /**
     * Búsqueda filtrable por campo dentro de un tipo de producto.
     *
     * @param termino texto a buscar (se hace LIKE %termino% en minúsculas)
     * @param idTipo  identificador del {@code TipoProducto}
     * @param campo   uno de: {@code "TODOS"}, {@code "CODIGO"}, {@code "NOMBRE"},
     *                {@code "INVIMA"}, {@code "LOTE"}, {@code "UBICACION"},
     *                {@code "PRESENTACION"}. Cualquier otro valor se trata como TODOS.
     */
    public List<Object[]> buscarConLotesPorCampo(String termino, int idTipo, String campo) {
        List<Object[]> lista = new ArrayList<>();
        String pat = "%" + (termino == null ? "" : termino.toLowerCase()) + "%";
        String filtro;
        int placeholders;

        String c = campo == null ? "TODOS" : campo.toUpperCase();
        switch (c) {
            case "CODIGO":       filtro = "LOWER(p.codigo) LIKE ?";        placeholders = 1; break;
            case "NOMBRE":       filtro = "LOWER(p.nombre) LIKE ?";        placeholders = 1; break;
            case "INVIMA":       filtro = "LOWER(p.invima) LIKE ?";        placeholders = 1; break;
            case "LOTE":         filtro = "LOWER(le.lote) LIKE ?";         placeholders = 1; break;
            case "UBICACION":    filtro = "LOWER(COALESCE(u.descripcion,'')) LIKE ?"; placeholders = 1; break;
            case "PRESENTACION": filtro = "LOWER(pr.descripcion) LIKE ?";  placeholders = 1; break;
            default: // TODOS
                filtro =
                    "(LOWER(p.nombre) LIKE ? OR LOWER(p.codigo) LIKE ? OR LOWER(p.invima) LIKE ? " +
                    " OR LOWER(le.lote) LIKE ? OR LOWER(COALESCE(u.descripcion,'')) LIKE ? " +
                    " OR LOWER(pr.descripcion) LIKE ?)";
                placeholders = 6;
        }
        ...
        final String sql =
            ...
            "LEFT JOIN LoteExistencia le ON p.id_producto = le.id_producto " +
            "LEFT JOIN Ubicacion u       ON le.id_ubicacion = u.id_ubicacion " +
            "WHERE p.id_tipo = ? AND " + filtro + " " +
            "ORDER BY p.nombre, le.fecha_vencimiento";
        try (PreparedStatement ps = ConexionBD.getInstance().getConexion().prepareStatement(sql)) {
            ps.setInt(1, idTipo);
            for (int i = 0; i < placeholders; i++) ps.setString(2 + i, pat);
            ...
            LOGGER.log(Level.SEVERE, "Error al buscar productos por campo " + c, e);
        }
        return lista;
    }
