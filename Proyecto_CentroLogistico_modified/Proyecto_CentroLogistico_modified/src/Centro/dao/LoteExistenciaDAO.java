...
public class LoteExistenciaDAO {
    ...
    /**
     * Normaliza al formato {@code DD/MM/YYYY} cualquier fecha de vencimiento
     * almacenada en formato ISO ({@code YYYY-MM-DD}). Idempotente y seguro.
     * @return número de filas actualizadas
     */
    public int normalizarFormatoFechas() {
        String sql =
            "UPDATE LoteExistencia " +
            "SET fecha_vencimiento = substr(fecha_vencimiento,9,2) || '/' || " +
            "                        substr(fecha_vencimiento,6,2) || '/' || " +
            "                        substr(fecha_vencimiento,1,4) " +
            "WHERE fecha_vencimiento IS NOT NULL " +
            "  AND length(fecha_vencimiento) = 10 " +
            "  AND substr(fecha_vencimiento,5,1) = '-' " +
            "  AND substr(fecha_vencimiento,8,1) = '-'";
        try (Statement st = ConexionBD.getInstance().getConexion().createStatement()) {
            return st.executeUpdate(sql);
        } catch (SQLException e) {
            LOGGER.log(Level.WARNING, "No se pudo normalizar el formato de fechas", e);
            return 0;
        }
    }
}
